...
\begin{document}
  ...
  \begin{algorithm}
    ...
    \KwResult{Trainieren der Critic-Parameter
  ...
  \begin{algorithm}
    ...
    \KwIn{Gradient-Penalty-Koeffizient $\lambda$, Anzahl von Critic-Iterationen
    $n_{critic}$, Batch-Größe $m$, Adam-Hyperparameter $\alpha, \beta_1,
    \beta_2$.}
    \KwResult{Trainieren der Critic-Parameter $w$ und Generator-Parameter
    $\theta$.}
    \BlankLine
    \While{$\theta$ \textnormal{ist nicht konvergiert}}{
      \For{$t = 1, ..., n_{critic}$}{
