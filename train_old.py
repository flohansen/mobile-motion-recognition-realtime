...
train_images = load_video_data(dataset_dir, (256, 192), 80, use_full_videos=True)
...
def train(dataset, epochs):
    ...
    for epoch in range(epochs):
        ...
        noise = tf.random.normal([args.batch_size, noise_dim])
        test_frames = model.generator(noise, training=False)
        test_frame = np.array(test_frames[0, 0, :, :, :] * 127.5 + 127.5).astype('uint8').reshape(-1, 192, 256, 3)
        ...
        with train_summary_writer.as_default():
            ...
            tf.summary.image('Generated frame', test_frame, step=epoch)
